...
onload = async ()=>{
    let filename = request.getUrlParams(decodeURI(location.href.split("#")[0])).file;
    ...
    switch (result.code) {
        case 200:
            document.querySelector(".md_info").innerHTML = `创建：${reader.FnewDate(result.metadata.ctime)} &nbsp;&nbsp;浏览：${result.metadata.look+1} &nbsp;&nbsp;收藏：${result.metadata.like} &nbsp;&nbsp;修改：${reader.FnewDate(result.metadata.mtime)}`;
            reader.showMd(result.content);
            break;
        case 429:
            console.log("过多请求");
...
{let self = window.reader = {
    ...
    setTitleNum:function(emName){
        ...
        self.observTitle(config.directory);
    },
    setDirectory:function(data){
        let dirEm = document.querySelector("header");
        let content = "";
        for(var i=0;i<data.length;i++){
            content += `<div onclick = "reader.pScrollTo('#${data[i].anchor}')" class="${'p_h'+(data[i].anchor.length)/2}" data-anchor="${data[i].anchor}" title="${data[i].title}">${data[i].text}</div>`
        }
        dirEm.innerHTML += content;
    },
    pScrollTo:function(idName){
        //self.changeHase(idName);
    ...
    observTitle:function(directory){
        ...
        let observer = new IntersectionObserver(function(e){
            ...
            clearTimeout(self.timers.observer);
            self.timers.observer = setTimeout(function(){
                self.changeHase("#"+e[0].target.id);
            },500);
            self.activeEm(active);
        },options);
        for(var i=0;i<directory.length;i++){
            observer.observe(document.querySelector("#"+directory[i].anchor));
        }
    },
    changeHase:function(hase){
        let url = location.href.split("#")[0]+hase;
    ...
    setCodeBtn:function(emName){
        ...
        for(let i=0;i<preCodeList.length;i++){
            ...
            div.innerHTML = `<span onclick="reader.switchCodeView(this)"><i class="fa fa-angle-down" aria-hidden="true" /></i>${getLanguage(preCodeList[i])}</span><button onclick="copyCode(this)"><i class="fa fa-copy" aria-hidden="true" /></i> 复制</button>`;
